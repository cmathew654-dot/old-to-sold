            ...
            {consignmentsError ? (
              <Alert className="border-red-200 bg-red-50">
                ...
                <AlertDescription className="text-red-800">
                  Failed to load consignments: {consignmentsError}
                </AlertDescription>
              </Alert>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-3">
                          <h3 className="font-semibold text-neutral-900">
                            {consignment.payload?.itemTitle || "Untitled Item"}
                          </h3>
                          <Badge variant="outline" className="text-xs">
                            {consignment.payload?.condition || "N/A"}
                          </Badge>
                        </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <Card className="max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <CardHeader>
              ...
              <CardDescription>
                Submitted {formatDistanceToNow(new Date(selectedConsignment.created_at), { addSuffix: true })}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <div>
                      <strong>Name:</strong> {selectedConsignment.name}
                    </div>
                    <div>
                      <strong>Email:</strong> {selectedConsignment.email}
                    </div>
                    {selectedConsignment.phone && (
                      <div>
                        <strong>Phone:</strong> {selectedConsignment.phone}
                      </div>
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <div>
                      <strong>Title:</strong> {selectedConsignment.payload?.itemTitle || "N/A"}
                    </div>
                    <div>
                      <strong>Condition:</strong> {selectedConsignment.payload?.condition || "N/A"}
                    </div>
                    <div>
                      <strong>Method:</strong> {selectedConsignment.payload?.method || "N/A"}
                    </div>
                  </div>
                </div>
              </div>
